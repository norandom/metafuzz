...
class ProductionClient < EventMachine::Connection
    ...
    def self.setup( config_hsh={})
        ...
        @server_waits=[]
        @case_id=0
        @template_hash=Digest::MD5.hexdigest(self.class.template)
        class << self
            attr_accessor 'case_id', 'idtracker', 'server_waits', 'template_hash'
        end
    end
    ...
    def send_client_startup
        ...
        raw_template=self.class.template
        begin
            ...
            when 'base64'
                encoded_template=Base64.encode64 raw_template
            when 'none'
                encoded_template=raw_template
            else
                encoded_template=Base64.encode64 raw_template
            end
        rescue
